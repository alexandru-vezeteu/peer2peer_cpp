...
struct choice
{
	[[nodiscard]] constexpr bool unwrap_public() const noexcept
	{
		return val != 0;
	}

	constexpr choice operator!() const noexcept
	{
		return choice(uint8_t(val ^ 0xFF));
	}
	constexpr choice operator&&(choice o) const noexcept
	{
		return choice(uint8_t(val & o.val));
	}
	constexpr choice operator||(choice o) const noexcept
	{
		return choice(uint8_t(val | o.val));
  ...
  public:
	...
	static constexpr choice true_choice() noexcept
	{
		return choice{uint8_t(0xFF)};
	}
	static constexpr choice false_choice() noexcept
	{
		return choice{uint8_t(0x00)};
	}
  ...
  private:
	...
	// very bad idea ngl
	constexpr choice(uint64_t diff) noexcept
	{
